...
(defafun create-turtl-db (future) (name)
  ...
  (log:debu1 "schema: create/verify table ~a" name)
...
(defafun apply-indexes (future) (table schema &key db)
  ...
  (alet* ((table-name (schema-name-to-string table))
    ...
    (log:debu1 "schema: existing indexes: ~s ~s" (car indexes) (funcall remove-version-fn (car indexes)))
    ...
    (let ((to-add nil)
      ...
      (labels ((add-index (index-key)
        ...
        (log:debu1 "schema: applying indexes (add)" to-add)
        (wait-for (adolist (index-key to-add)
                    (add-index index-key))
          (log:debu1 "schema: applying indexes (delete)" to-delete)
          (wait-for (adolist (index-name to-delete)
                      (delete-index index-name))
            (log:debu1 "schema: applying indexes (edit)" to-edit)
            (wait-for (adolist (entry to-edit)
                        (let ((index-key (car entry))
                              (old-index-name (cdr entry)))
                          (edit-index old-index-name index-key)))
              (r:disconnect sock)
              (log:debu1 "schema: indexes applied")
              (let ((report nil))
